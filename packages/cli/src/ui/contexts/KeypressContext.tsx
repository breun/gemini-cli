...
export function KeypressProvider({
  ...
  useEffect(() => {
    ...
    const couldBeKittySequence = (buffer: string): boolean => {
      // Empty buffer could be anything
      if (!buffer) return true;

      // If the buffer is ESC alone that could be a kitty sequence but risks
      // delaying normal ESC key handling, so we treat it as non-kitty and
      // assume that kitty sequences will include at least the first two
      // characters in the same chunk.
      if (buffer === `${ESC}[`) return true;
